...
class Timetable():
    ...
    async def send_lesson_embed(self,*args):
        ...
        if self.is_lesson_in_holiday(GroupID,LessonDatetime):
    ...
    async def send_lesson_countdown(self,*args):
        ...
        if self.is_lesson_in_holiday(GroupID,WarningDatetime):
            return # No lessons during a holiday so no need to carry on with this function
    ...
    def is_lesson_in_holiday(self,GroupID,DateTimeInput) -> bool:
        """
    ...
    async def update_time_channels(self,GroupID: int):
        """
        ...
        """
        NextLesson = self.get_next_lesson((GroupID,))
        GroupInfo = db.records("SELECT * FROM LessonGroups WHERE GroupID = ?",GroupID)
        ...
        await self.bot.rest.edit_channel(NLDayID,NLDayStr)
        await self.bot.rest.edit_channel(NLTimeID,NLTimeStr)
    ...
    def get_group_ids_from_user(self,UserID) -> tuple or None:
        ...
        any groups.
        """
        GroupIDs = db.records("SELECT GroupID FROM Students WHERE UserID = ?",UserID)
        if GroupIDs == []:
            return None
        else:
            return GroupIDs
    ...
    def get_group_info_from_input(self,Group_Input) -> tuple or None:
        ...
        This input could be a GroupCode, GroupName or a GroupID
        Similar to above, it will return the group info of the selected group
        """
        pass
    def get_next_lesson(self,GroupIDs: tuple[int]) -> tuple or None:
        """
    ...
    def get_assignments_for_lesson(self, lesson: list[str]) -> list[str] or None:
        """
